...
interface HomeProps {
  initialData: {
    ...
    mode2Note?: string; // ✨ Naya Note Mode 2 ke liye
  };
}
...
{/* Main Content */}
                     ...
                     <div className="mb-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
                       <div className="relative w-full rounded-3xl p-[3px] animate-rainbow shadow-[0_0_60px_rgba(255,255,255,0.05)]">
                         <div className="bg-[#0f0f0f] rounded-[21px] w-full p-6 sm:p-10 relative overflow-hidden h-full z-10 border border-white/5">
                           ...
                           {/* ✨ MODE 2 NOTE (Categories Page) ✨ */}
                           {initialData.mode2Note && (
                             <div className="mt-8 text-center relative z-10">
                               <p className="inline-block text-sm sm:text-base text-red-200 border border-red-500/30 bg-red-900/20 px-4 py-2 rounded-lg backdrop-blur-sm shadow-md">
                                 {initialData.mode2Note}
                               </p>
                             </div>
                           )}
        ...
        <Script src="https://pl28382929.effectivegatecpm.com/b1/06/0e/b1060e51e3f0ca4c6da303d42b6ea068.js" strategy="afterInteractive"/>
      </div>
    </>
  );
}
...
// 2 done bas mode2note add karty hai 
...
// ============== Alhamdullah yeh bilkul teek hai yeh sab teek hai iss me srf eek hey mode hai mode1 jukey ok.ru . Abb hum mode 2 bey add karty hai jissmey iframe hugy ==================
...
//                            <div className="flex flex-wrap gap-3 mt-1 relative z-10">
//                              {evaluatedChannels.map((evalData, idx) => {
//                                  // ✨ DISABLED EXPIRED BUTTON UI UPDATE
//                                  const isExpired = evalData.state === 'EXPIRED';
//                                  const isActive = activeVideoId === evalData.channel.videoId;
...
//                                      {/* TEXT LOGIC (Strike-through if expired) */}
//                                      <span className={`relative z-10 tracking-wide uppercase flex flex-col sm:flex-row sm:items-center sm:gap-2 ${isExpired ? 'line-through decoration-red-800/80 decoration-2' : ''}`}>
//                                         {evalData.displayName}
//                                      </span>
...
// yeh below code iss format k liyee tha .      "name": "Wolves vs Port Vale||S=2026-08-06T23:45:00+05:00||E=2026-08-07T03:45:00+05:00", yeh data.json me hai 
...
//   // =========================================================
//   // 🚀 ISO-8601 UNIFIED SCHEDULER ENGINE
//   // Format: !Match Name||S=2026-08-06T23:30:00+05:00||E=2026-08-07T02:15:00+05:00
//   // =========================================================
//   useEffect(() => {
//     if (!selectedVideo || !selectedVideo.channels) return;
...
//                            <div className="flex flex-wrap gap-3 mt-1 relative z-10">
//                              {evaluatedChannels.map((evalData, idx) => {
//                                  // HIDE EXPIRED MATCHES FROM THE SERVER LIST
//                                  if (evalData.state === 'EXPIRED') return null;
...
//                                      {/* 🎨 UI displays clean name + formatted time */}
//                                      <span className="relative z-10 tracking-wide uppercase">{evalData.displayName}</span>
...
// ============= !!!!!!!!!!!!!! ALl done alhamdulllah, bas aab security add karty hai taakey koi ok.ru means hamara warehouse live kaa deek n payee etney asaaney see ====================
...
// ========== ALhamdullah sab teek hai bas woo iframe(video player ko center me karna hai upper code me ====================
...
// Bohot hi zabardast point uthaya aapne! Main samajh gaya. Aapka matlab hai ke jab user pehli baar website par aaye aur kisi match par click kare, toh Player par Red Play Button (Overlay) zaroor aana chahiye (Paused state). Lekin jab wo already player dekh raha ho aur neeche se koi Available Server ya Bottom Stream select kare, toh phir direct Play (Auto-play) ho jaye.

// Yeh bilkul logical aur professional approach hai. Isko fix karne ke liye hume HomeClient.tsx mein ek chota sa state (forceAutoPlay) lagana hoga.

// Yahan dekhein kya logic apply hogi:

// Jab user Main Screen (Top) se koi match select karega 👉 forceAutoPlay = false (Red button aayega).

// Jab user Available Servers ya Bottom Links par click karega 👉 forceAutoPlay = true (Direct chalega).

// Yeh raha aapka completely fixed HomeClient.tsx (aapko OkRuPlayer.tsx mein mazeed koi change nahi karna, wo pichla wala hi theek hai).
...
// 1 iss below code mei joo change keya hai uss k waja see okRuPlayer.tsx file me bey change keya hai ok src/components/OkRuPlayer.tsx
// iss below code me yeh hai k jab websiet par ata hai direct player start huta hai .
...
// iss bottom code ko remove karoo jab bey aap yeh deko yeh line ok.
