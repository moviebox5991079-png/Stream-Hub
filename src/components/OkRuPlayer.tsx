...
// Yeh raha aapka bilkul safe aur updated code. Maine aapki kisi bhi logic (fullscreen, orientation, shields, loading state) ko bilkul nahi chhera. Bas embedUrl bananay ke tareeqay mein Base64 Security Layer add kar di hai.
...
const OkRuPlayer: React.FC<OkRuPlayerProps> = ({ 
  ...
  thumbnail,
  autoPlay = false,
  forcePlayOnLoad = false
}) => {
  const containerRef = useRef<HTMLDivElement>(null);
  
  // ✨ Initial state ab prop se control hogi
  const [isPlayClicked, setIsPlayClicked] = useState<boolean>(forcePlayOnLoad); 
  ...
  // 🔒 YAHAN SECURITY FIX APPLY KIYA HAI (Base64 Injection)
  const embedUrl = useMemo(() => {
    if (!cleanVideoId) return '';
    
    // 'aHR0cHM6Ly9vay5ydS92aWRlb2VtYmVkLw==' ko decode karega toh 'https://ok.ru/videoembed/' banega
    // typeof window check ensures Next.js SSR (Server) par error na aaye
    const hiddenDomain = typeof window !== 'undefined' 
      ? window.atob('aHR0cHM6Ly9vay5ydS92aWRlb2VtYmVkLw==') 
      : 'https://ok.ru/videoembed/'; 
      
    return `${hiddenDomain}${cleanVideoId}?nochat=1&autoplay=1`;
  }, [cleanVideoId]);
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center z-0 bg-[#000000] overflow-hidden">
                ...
                <div className="relative flex flex-col items-center gap-6 z-10">
                  ...
                  <div className="flex flex-col items-center gap-1">
                    <span className="text-white font-bold tracking-[0.2em] text-sm animate-pulse">Loading....</span>
...
//   // ✨ FIX APPLIED HERE: Initial state ab prop se control hogi
//   const [isPlayClicked, setIsPlayClicked] = useState<boolean>(forcePlayOnLoad); 
...
//   // Autoplay=1 fixed hai kyunke ab user manually click kar raha hai (ya bottom list se trigger ho raha hai)
...
// ============== Alhamdullah yeh teek ai bas eek or shield add karna hai top left side par jaha par video k title hai woo redirect karta hai isko chupana hai =========================
